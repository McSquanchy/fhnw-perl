...
# This version uses text colouring to indicate blank tiles.
...
# Print out colourized matches, sorted alphabetically by score then length...
say {$pager} $_ for map  { sprintf("%3d  %s", score($_), colourize($_)) }
...
# Highlight blanks by colour...
sub colourize ($word) {
    $word =~ s{ ([[:lower:]]) }{ colored(uc $1, @HIGHLIGHT) }xge;
    return $word;
}
